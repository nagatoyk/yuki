...
loader.add('BG1','/lab/_assets/82_res.images.ImgBackgroundDay.jpg')
	.add('BG2','/lab/_assets/83_res.images.ImgBackgroundNight.jpg')
	.add('shutter1','/lab/_assets/513_res.common.ImgShutterTopDark.png')
	.add('shutter2','/lab/_assets/511_res.common.ImgShutterBottomDark.png')
	.add('radar','/lab/_assets/375_res.battle.images.ImgRaderBG.png')
	.add('nmiss','/lab/_assets/Nmiss.png')
	.add('ccrit','/lab/_assets/Ccrit.png')
	.add('dminor','/lab/_assets/d383.png')
	.add('dmedium','/lab/_assets/d385.png')
	.add('dmajor','/lab/_assets/d387.png')
	.add('938','/lab/_assets/938.png')
	.add('914','/lab/_assets/914.png')
	.add('916','/lab/_assets/916.png')
	.add('918','/lab/_assets/918.png')
	.add('920','/lab/_assets/920.png')
	.add('922','/lab/_assets/922.png')
	.add('924','/lab/_assets/924.png')
	.add('926','/lab/_assets/926.png')
	.add('440','/lab/_assets/440.png')
	.add('445','/lab/_assets/445.png')
	.add('447','/lab/_assets/447.png')
	.add('449','/lab/_assets/449.png')
	.add('433','/lab/_assets/433.png')
	.add('488','/lab/_assets/488.png')
	.add('493','/lab/_assets/493.png')
	.add('498','/lab/_assets/498.png')
	.add('465','/lab/_assets/465.png')
	.add('467','/lab/_assets/467.png')
	.add('469','/lab/_assets/469.png')
	.add('471','/lab/_assets/471.png')
	.add('473','/lab/_assets/473.png')
	.add('216','/lab/_assets/216.png')
	.add('aaci1','/lab/_assets/aaci1.png')
	.add('aaci2','/lab/_assets/aaci2.png');
for (var i=389; i <= 417; i+=2) loader.add(i.toString(),'/lab/_assets/'+i+'.png');
for (var i=0; i<=9; i++) loader.add('C'+i,'/lab/_assets/C'+i+'.png');
for (var i=0; i<=9; i++) loader.add('N'+i,'/lab/_assets/N'+i+'.png');
var ALLLOADED = false;
loader.load(function() { ALLLOADED = true; });

// create a new Sprite using the texture
var bg = PIXI.Sprite.fromImage('/lab/_assets/82_res.images.ImgBackgroundDay.jpg');
var bg2 = PIXI.Sprite.fromImage('/lab/_assets/83_res.images.ImgBackgroundNight.jpg');

// stage.addChild(bunny);
stage.addChild(bg);

var frames_exp = [];
for (var i=389; i <= 417; i+=2) frames_exp.push(PIXI.Texture.fromImage('/lab/_assets/'+i+'.png'));
...
var radar1 = PIXI.Sprite.fromImage('/lab/_assets/375_res.battle.images.ImgRaderBG.png');
radar1.position.set(75,402);
radar1.anchor.set(.5);
radar1.scale.set(0);
stage.addChild(radar1);

var radar2 = PIXI.Sprite.fromImage('/lab/_assets/375_res.battle.images.ImgRaderBG.png');
...
function createDots(container,form,num,side) {
	var name, path;
	if (side) { name = 'dotRed'; path = '/lab/_assets/467.png'; } //change path
	else { name = 'dotGreen'; path = '/lab/_assets/465.png'; }
	form = parseInt(form);
	switch(form) {
		case 0:
		case 1:
		...
		case 11:
			...
			for (var i=0; i<coords.length; i++) {
				var dot = getFromPool('dotGreenC','/lab/_assets/473.png');
				dot.position.set(coords[i][0],coords[i][1]);
				dot.anchor.set(.5);
				container.addChild(dot);
			}
			...
			for (var i=0; i<coords.length; i++) {
				var dot = getFromPool((COMBINED==2)?'dotYellow':'dotBlue',(COMBINED==2)?'/lab/_assets/469.png':'/lab/_assets/471.png');
				dot.position.set(coords[i][0],coords[i][1]);
				dot.anchor.set(.5);
				container.addChild(dot);
			}
...
var shutterTop = PIXI.Sprite.fromImage('/lab/_assets/513_res.common.ImgShutterTopDark.png');
var shutterBottom = PIXI.Sprite.fromImage('/lab/_assets/511_res.common.ImgShutterBottomDark.png');
...
function createShip(data,side,i) {
	...
	var portrait = PIXI.Sprite.fromImage('/lab/_assets/icons/'+sdata.image);
	...
	var hpbarback = PIXI.Sprite.fromImage('/lab/_assets/433.png');
	hpbarback.position.set((side==1)?2:160,2);
	graphic.addChild(hpbarback);
	graphic.addChild(hpbar);
	graphic.addChild(portrait);
	if (i==0) {
		var box = PIXI.Sprite.fromImage('/lab/_assets/'+((side==1)?'449':'445')+'.png');
		box.y -= 5;
		graphic.addChild(box);
	} else graphic.addChild(PIXI.Sprite.fromImage('/lab/_assets/'+((side==1)?'447':'440')+'.png'));
	graphic.addChild(hptxt);
	graphic.position.set((side==1)?FLEET2ORIGIN+220:-220,((side==1)?144:77)+45*i);
	ship.graphic = graphic;
	// var mask = PIXI.Sprite.fromImage('/lab/_assets/mask.png');
...
function processAPI(root) {
	...
	if (root.combined) {
		for (var i=0; i<fshipsC.length; i++) {  //create ship objects (combined)
			...
			var mask = new PIXI.Sprite.fromImage('/lab/_assets/mask.png');
...
function createNumber(x,y,number) {
	...
	if (number <= 0) {
		var sprite = getFromPool('Nmiss','/lab/_assets/Nmiss.png');
		...
		for (i=0; i<number.length; i++) {
			var j = 0;
			addTimeout(function(){
				var sprite = getFromPool('N'+number[j],'/lab/_assets/N'+number[j]+'.png');
		...
		for (i=0; i<number.length; i++) {
			var j = 0;
			addTimeout(function(){
				var sprite = getFromPool('C'+number[j],'/lab/_assets/C'+number[j]+'.png');
		...
		var crit = getFromPool('Ccrit','/lab/_assets/Ccrit.png');
		crit.x = -10;
		crit.y = 10;
		crit.iscrit = true;
		crit.anchor.set(.5);
...
function createTorp(ship,target,speed,big) {
	if (!speed) speed = 4;
	var torp = (big)? getFromPool('torpedoBig','/lab/_assets/493.png') : getFromPool('torpedo','/lab/_assets/498.png');
...
function shipSetHP(ship,hp) {
	...
	if (hp <= 0) {
		if (ship.status != 0) {
			...
			var dam = PIXI.Sprite.fromImage('/lab/_assets/d389.png'); dam.y += 2;
		...
		if (ship.status != 1) {
			if (ship.status < 4) ship.graphic.removeChildAt(5);
			var dam = PIXI.Sprite.fromImage('/lab/_assets/d387.png'); dam.y += 2;
			ship.graphic.addChild(dam);
			ship.damg = dam;
			if (ship.side == 1) ship.graphic.getChildAt(5).x += 10;
			ship.status = 1;
		...
		if (ship.status != 2) {
			if (ship.status < 4) ship.graphic.removeChildAt(5);
			var dam = PIXI.Sprite.fromImage('/lab/_assets/d385.png'); dam.y += 2;
			ship.graphic.addChild(dam);
			ship.damg = dam;
			if (ship.side == 1) ship.graphic.getChildAt(5).x += 10;
			ship.status = 2;
		...
		if (ship.status != 3) {
			if (ship.status < 4) ship.graphic.removeChildAt(5);
			var dam = PIXI.Sprite.fromImage('/lab/_assets/d383.png'); dam.y += 2;
			ship.graphic.addChild(dam);
			ship.damg = dam;
			if (ship.side == 1) ship.graphic.getChildAt(5).x += 10;
			ship.status = 3;
...
function createPlane(x,y,planetypes,shots) {
	...
	if (num == 3) {
		for (var i=0; i<3; i++) {
			var plane = PIXI.Sprite.fromImage('/lab/_assets/'+PLANESPRITES[planetypes[i]-1]+'.png');
			plane.x = i*25-25;
			plane.y = (i==1) ? -15 : 15;
			plane.scale.set(.8);
		...
		var plane = PIXI.Sprite.fromImage('/lab/_assets/'+PLANESPRITES[planetypes[0]-1]+'.png');
		plane.scale.set(.8);
		planes.addChild(plane);
...
function createAAfire(x,y,angle) {
	var fire = PIXI.Sprite.fromImage('/lab/_assets/216.png');
	fire.pivot.y = 5;
...
function createAACIfire(x,y) {
	var firefront = PIXI.Sprite.fromImage('/lab/_assets/aaci1.png');
	var fireback = PIXI.Sprite.fromImage('/lab/_assets/aaci2.png');
...
function createSupportShell(x,y,angle) {
	var shell = getFromPool('supportShell','/lab/_assets/488.png');
	shell.alpha = 1;
	shell.scale.set(.5);
