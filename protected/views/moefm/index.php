...
function set_login() {
	...
	link_right_user.getElementsByTagName('a')[1].addEventListener('click', function() {
		login()
	})
	link_right_user.getElementsByTagName('a')[2].addEventListener('click', function() {
			wb_login()
		})
		//start();
}
	...
	div.getElementsByClassName('login_confirm')[0].addEventListener('click', function() {
		...
		var url = 'http://api.moefou.org/oauth/request_token',
			options = {
				...
				consumerKey: ck,
				consumerSecret: cs,
				callback: '<?php echo Yii::app()->request->hostinfo.$this->createUrl('
				moefmcallback '); ?>'
			},
...
function wb_check() {
	weibojs = 'weibojs_<?php echo Yii::app()->params['
	saet_api_key ']; ?>';
